...
- **콘솔 로깅 병목 개선 + 동시성 버그 발견/수정**: 콘솔 로그 출력을 전용 워커 스레드로 분리해 300개 동시 접속 전원 성공(250→300/300) 달성, 단 워커 스레드 증가로 스레드 간 락 경합(`futex`)이 새 지배적 비용(57%)으로 떠오르는 트레이드오프 재확인. 재검증 중 `sync_with_stdio(false)`+다중 스레드 동시 쓰기로 인한 콘솔 로그 데이터 레이스를 직접 계측으로 발견·수정(성능 지표엔 영향 없음을 재실측으로 검증). 300-agent 규모의 남은 병목(스레드 락 경합)은 단일 프로세스/개발 머신 스펙의 현실적 한계로 판단해 수평 확장(다중 Collector)을 정공법으로 제시하고 이번 스케일 테스트 라운드를 마무리. 상세: `Docs/PROJECT_TECHNICAL_REVIEW.md` §7-7, 버그 8
- **실행 검증(`/apm/alerts`, `/apm/traces`)**: 그동안 빌드/단위테스트만 검증되고 실제로 띄워본 적 없던 알림/트레이스 페이지를 Collector+Console을 함께 실행해 처음 검증 — 그 과정에서 EF Core+SQLite가 `DateTimeOffset` 부등호 비교를 SQL로 번역 못 해 (1) 보존 정책 백그라운드 서비스가 시작 즉시 앱 전체를 크래시시키고 (2) `/apm/traces`가 항상 500을 반환하던 버그 2건을 발견해 수정. 임계치 편집 저장과 실제 알림 발생(임계치 초과 → 저장 → 렌더링)까지 실 트래픽으로 end-to-end 확인. 상세: `Docs/PROJECT_TECHNICAL_REVIEW.md` 버그 9·10
