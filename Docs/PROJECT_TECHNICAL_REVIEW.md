...
- [9. 발견한 버그 전체 (10건 상세)](#9-발견한-버그-전체-10건-상세)
- [10. 테스트 전략](#10-테스트-전략)
...
### 7-7. 콘솔 로깅 워커 분리 + 재검증 중 발견한 데이터 레이스 — "고쳤다"를 다시 의심해보기

**배경**: 7-5에서 찾아뒀던 콘솔 로깅 병목(`PacketHandler::Register` 핸들러의 `std::cout << ... << std::endl`, 300-agent에서 write syscall 92%)을 실제로 고쳤다. 로그 조립/출력을 전용 워커 스레드(`consoleLogQueue`)로 위임하고, 워커 안에서도 `std::endl`(강제 flush) 대신 `'\n'`을 써서 처리 비용 자체를 낮췄다 — `WorkerQueue` 내부 큐가 무제한이라, flush 비용을 안 줄이고 스레드만 옮기면 유입 속도가 처리 속도를 앞지를 때 메모리가 계속 쌓일 수 있기 때문(7-6절 WAL 교훈과 같은 형태의 리스크를 미리 차단).

**재실측(300-agent, `strace -f -c`, WAL 적용 후 대비)**:

| 지표 | WAL 적용 후 | 로깅 워커 분리 후 | 비고 |
|---|---|---|---|
| connect_success | 250/300 | **300/300(전원)** | 목표 달성 |
| latency p95/p99 | 19,890ms/26,044ms | 19,322ms/24,970ms | 더 많은 트래픽을 처리하면서도 악화 안 됨 |
| futex | 18.19초(36.5%) | **49.26초(57.22%)** | **새로 드러난 병목** |
| 전체 syscall 시간 | 49.81초 | 86.09초 | 처리량 증가 + futex 경합 증가가 겹침 |

**발견 — 이번에도 트레이드오프였다**: 접속 성공률/지연시간이라는 체감 지표는 확실히 좋아졌지만(300개 전원 접속 성공), 워커 스레드가 1개(`metricStoreQueue`)에서 2개(+`consoleLogQueue`)로 늘면서 스레드 간 락 경합(`futex`)이 새 지배적 비용(57%)으로 떠올라 메트릭당 정규화 syscall 비용은 오히려 늘었다(0.345ms→0.501ms) — 7-6절 WAL의 `fdatasync`↔`fcntl` 트레이드오프와 같은 패턴이 "스레드 개수" 축에서도 재현됐다.

**한 번 더 뒤집힌 결론 — 검증하려다 새 버그를 찾다**: 이 개선을 100-agent 규모에서도 확인하려던 중, `strace -c`(메인 스레드만 추적)에서 여전히 `write`가 지배적으로 나와 의아해서 스레드 ID를 직접 계측(`std::this_thread::get_id()`)해봤다. 그 진단 로그 자체가 `[DIAG] log thread=[DIAG] store thread=...`처럼 스레드 간 문자 단위로 뒤섞여 나왔다 — `consoleLogQueue` 워커 스레드 분리 자체는 정확했지만(스레드 ID가 실제로 서로 달랐음), 네트워크 스레드에 남겨둔 나머지 콘솔 로그(`connection accepted`, `WebServer로 N건 전송 시도` 등)가 여전히 같은 `std::cout`/`std::cerr`를 직접 썼고, 함께 넣었던 `sync_with_stdio(false)`가 C stdio의 내부 락(멀티스레드 안전장치)을 없애면서 이 동시 접근이 실제 데이터 레이스로 이어졌다. 남은 콘솔 출력도 전부 `consoleLogQueue`로 위임해 "런타임 중엔 오직 한 스레드만 스트림을 건드린다"는 원칙을 전체에 일관 적용해 해소했다. 재검증 결과 이 수정은 성능 지표 자체는 바꾸지 않았다(100-agent p99 2,049ms→2,053ms, 300-agent 전체 syscall 시간 86.09초→88.45초, 오차범위 내) — 순수하게 로그 텍스트 무결성만의 문제였다.

**100-agent 기준으로 좁혀보면**: 72→100 하드 리밋 해소는 이미 7-5(WorkerQueue)에서 끝나 있었고, 이번에 추가한 WAL+로깅 개선은 워커 스레드를 1개→3개로 늘리며 오히려 p99를 소폭 늘렸다(1,010ms→2,053ms) — 이 개선들의 실질 효과는 300-agent 같은 고부하 구간에 있고, 100-agent 규모에선 순효과가 거의 없거나 근소한 손해라는 게 정직한 평가다.

**최종 결론(2026-07-29 확정)**: 300-agent 시나리오에 남은 futex 경합(스레드 간 락 대기)은 이번 프로젝트 범위에서 더 파고들지 않기로 했다. 단일 Collector 프로세스 하나로 300개 동시 접속을 완전히 무손실로 처리하는 건, 지금 쓰고 있는 개발 머신 한 대의 스펙 안에서는 현실적인 한계에 가깝다고 판단했고, 실제 운영이라면 이 지점부터는 코드를 더 쥐어짜기보다 Collector를 여러 대로 수평 확장(로드밸런서 뒤에 N대)하는 게 정공법이라는 결론으로 이번 스케일 테스트 라운드를 마무리한다.
...
- **Q. 콘솔 로깅 병목은 어떻게 고쳤고, 그 결과는 어땠나요?**
  → "로그 조립/출력을 전용 워커 스레드로 옮기고, 워커 안에서도 `std::endl`(강제 flush) 대신 `'\n'`을 써서 처리 비용 자체를 낮췄습니다. 재실측 결과 300-agent 접속 성공률이 250/300에서 300/300(전원)으로 올라갔고 지연시간도 악화되지 않았지만, 워커 스레드가 1개에서 2개로 늘면서 스레드 간 락 경합(futex)이 새로운 지배적 비용(57%)으로 떠올라 전체 syscall 시간은 오히려 늘었습니다 — WAL 때와 같은 '트레이드오프' 패턴이 스레드 개수 축에서도 재현된 사례입니다."
- **Q. 이 개선을 검증하다가 새로 발견한 버그가 있다고요?**
  → "네, 100-agent 규모에서 검증하려고 스레드 ID를 직접 계측하는 진단 코드를 넣었는데, 그 진단 로그 자체가 스레드 간에 문자 단위로 뒤섞여 나왔습니다. 로그 워커 스레드 분리 자체는 정확했지만, 네트워크 스레드에 남겨둔 다른 콘솔 로그 몇 곳이 여전히 같은 스트림에 직접 쓰고 있었고, 같이 넣었던 `sync_with_stdio(false)`가 멀티스레드 안전장치를 없애버려서 실제 데이터 레이스로 이어졌던 겁니다. 남은 로그 출력도 전부 같은 워커로 위임해서 '런타임 중엔 스트림을 오직 한 스레드만 건드린다'는 원칙을 일관되게 적용해 해결했습니다. 재검증해보니 이 수정 자체는 성능 지표를 바꾸지 않았고, 순수하게 로그 텍스트 무결성 문제였다는 걸 확인했습니다."
- **Q. 300-agent 성능 한계를 왜 더 파고들지 않고 여기서 멈췄나요?**
  → "남은 병목(futex, 스레드 간 락 경합)은 단일 프로세스 안에서 워커 스레드를 더 잘게 쪼개거나 락-프리 구조로 재설계해야 하는 영역이라, 들이는 노력 대비 얻는 게 적다고 판단했습니다. 단일 Collector 프로세스 하나로, 그것도 개발용 머신 한 대에서 300개 동시 접속을 완전히 무손실로 처리하는 건 애초에 그 규모의 하드웨어가 감당할 수 있는 현실적인 한계에 가깝습니다. 실제 운영이라면 이 지점부터는 코드를 더 쥐어짜기보다 Collector를 여러 대로 수평 확장하는 게 정공법이라, 그 결론으로 이번 스케일 테스트 라운드를 마무리했습니다."
...
이 프로젝트 전체를 통틀어 "제안 단계에선 맞아 보였지만 실제 빌드/실행/렌더링에서 발견된" 버그 10건. 전부 **왜 발견됐는지(어떤 검증 방법으로)**까지 같이 기억해두면 "디버깅 어떻게 하세요" 류 질문에 구체적으로 답할 수 있다.
...
### 버그 8 — `sync_with_stdio(false)` + 다중 스레드 `cout` 동시 쓰기 데이터 레이스
- **증상**: 콘솔 로그 텍스트가 스레드 간에 문자 단위로 뒤섞여 나옴(서로 다른 두 스레드의 로그 줄이 한 줄 안에 섞임). 저장된 메트릭 데이터나 성능 지표엔 영향 없음 — 순수하게 콘솔 출력 무결성 문제.
- **원인**: 콘솔 로깅 병목(7-5/7-7절)을 고치며 메트릭 로그는 전용 워커 스레드로 옮겼지만, 네트워크 스레드에 남아있던 다른 로그 호출(연결 수립, WebServer 전송 시도 등) 몇 곳이 여전히 같은 `std::cout`/`std::cerr`에 직접 씀 — 두 스레드가 같은 스트림에 동시 접근. 성능을 위해 같이 넣은 `std::ios::sync_with_stdio(false)`가 C stdio의 내부 락(멀티스레드 안전장치)을 없애면서, 원래는 우연히 안전했던 동시 접근이 실제 데이터 레이스로 이어짐.
- **발견 과정**: 100-agent 재검증 중 `strace -c`(메인 스레드만 추적)에서도 여전히 `write`가 지배적으로 나오는 게 의아해서, `std::this_thread::get_id()`를 출력하는 임시 진단 코드를 넣고 직접 계측하다가 그 진단 로그 자체가 깨져서 나오는 걸 목격 — 이론이 아니라 실측으로 재현.
- **수정**: 남은 콘솔 출력 호출도 전부 메트릭 로그와 같은 워커 큐로 위임 — 런타임 중엔 오직 그 워커 스레드 하나만 스트림을 건드리도록 통일.
- **검증**: 100-agent 스트레스 재현 테스트로 깨진 줄 0건 확인, 수정 전/후 성능 지표(연결 성공률/지연시간/syscall 시간)가 그대로임을 재실측으로 확인 — 순수 정확성 버그였지 성능 측정 오류가 아니었음을 검증.
- **교훈**: "이 최적화는 공짜"라고 판단했던 `sync_with_stdio(false)`가 사실은 스레드 안전성이라는 암묵적 전제를 깨는 트레이드오프였음 — 성능 최적화를 추가할 때마다 "이게 기존에 기대고 있던 다른 보장을 깨진 않는가"를 같이 점검해야 한다는 걸 재확인.

### 버그 9 — `RetentionService`가 시작 즉시 APM_Console 전체를 크래시시킴
- **증상**: `/apm/alerts`/`/apm/traces`(2·5순위)를 처음으로 실제 브라우저에서 열어보려고 `dotnet run`했더니, 요청을 한 번도 받기 전에 `Unhandled exception`으로 프로세스 자체가 죽음.
- **원인**: `RetentionService.PruneAsync()`의 `db.Metrics.Where(m => m.Ts < metricsCutoff).ExecuteDeleteAsync()`에서 `InvalidOperationException` — `Ts`가 `DateTimeOffset`인데, 이 EF Core+SQLite 프로바이더 조합이 `DateTimeOffset` 비교(`<`)를 SQL로 아예 번역하지 못함. `ExecuteDeleteAsync`뿐 아니라 일반 `Where(...).ToListAsync()`조차 똑같이 실패하는 걸로 확인돼, "ExecuteDelete 한정 제약"이 아니라 이 조합에서 `DateTimeOffset` 부등호 비교 자체가 안 되는 문제였음이 드러남. `RetentionService`는 시작 직후 실행되는 `BackgroundService`라 이 한 줄이 호스트 전체의 기동을 막았다.
- **발견 과정**: 2순위(알림)/5순위(트레이스) 작업 당시엔 "코드/빌드 관점에선 완료, 실행 관점 확인은 선택 사항"으로 미뤄뒀던 것 — 이번에 프로젝트를 마무리하며 실제로 실행해보다가 처음 발견. 3순위(보존 정책) 작업 때도 "필수는 아님"으로 실행 검증을 건너뛰었던 부분이라, 결국 **두 라운드 전에 만든 코드가 이번에야 처음 실행됐다.**
- **수정**: LINQ 번역을 아예 거치지 않는 파라미터화 raw SQL(`Database.ExecuteSqlInterpolatedAsync`, 값은 파라미터 바인딩이라 인젝션 안전)로 `Metrics`/`AlertRecords`/`TransactionSpans` 3개 DELETE문을 재작성. 상세 코드 전문: `SESSION_LOG.md` 2026-07-29 항목.
- **검증**: 재빌드 후 정상 기동 확인, EF Core 커맨드 로그로 파라미터 바인딩된 `DELETE FROM Metrics WHERE Ts < @p0` 등이 실제로 실행되는 것 확인. `dotnet test` 18/18 통과(회귀 없음).
- **교훈**: "코드/빌드 관점에선 완료, 실행 검증은 선택"이라고 두 차례(3순위, 2·5순위) 미뤄뒀던 판단이 실제로 위험했음이 증명된 사례 — 특히 시작 시 실행되는 백그라운드 서비스는 단 한 줄의 번역 실패가 앱 전체의 기동 자체를 막을 수 있어서, "이 경로는 리스크가 낮다"는 판단을 실행 검증 없이 내리면 안 된다는 교훈.

### 버그 10 — `/apm/traces`가 항상 HTTP 500
- **증상**: 버그 9를 고친 뒤 `/apm/alerts`는 정상 200이었지만 `/apm/traces`는 여전히 500.
- **원인**: `TracesController.Index()`의 `_db.TransactionSpans.Where(s => s.Ts >= cutoff)...ToListAsync()` — 버그 9와 정확히 같은 근본 원인(`DateTimeOffset` 부등호 비교가 SQL 번역 안 됨). 5순위 설계 당시 "OperationName+Ts 복합 인덱스가 이 WHERE에 맞는다"고 판단했던 전제가 실제로는 인덱스를 탈 수조차 없었던 셈.
- **발견 과정**: 버그 9를 고친 뒤 `curl`로 `/apm/traces` 응답 코드를 확인하다가 500을 보고 콘솔 로그의 스택 트레이스로 원인 확정 — 같은 원인이 또 있을 수 있다는 의심으로 `AlertsController`/`AlertEvaluator`도 같이 검색해봤는데, 그쪽은 `ClosedAt == null`/`!= null` 같은 동등 비교만 써서 이 버그의 영향을 안 받는다는 것도 함께 확인했다.
- **수정**: `Ts` 비교를 SQL로 안 보내고, `Ts` 컬럼까지 프로젝션에 포함해 전부 메모리로 가져온 뒤 C#에서 필터링하도록 변경 — 인덱스 활용은 포기하지만, 이 프로젝트 규모(보존 기간 30일)의 span 개수로는 감내 가능한 트레이드오프로 판단.
- **검증**: 재빌드 후 `window=1h/24h/7d` 전부 HTTP 200, 1h 창에서 LoadTester가 만든 실제 span 데이터(`Collector.HandleMetricPacket`)가 표에 나타나는 것 확인.
- **교훈**: 같은 근본 원인의 버그가 한 곳만이 아니라 "같은 패턴을 쓴 다른 코드"에도 있을 수 있다는 걸 가정하고, 고친 즉시 유사 코드를 능동적으로 재검색하는 습관이 실제로 두 번째 크래시를 앞서 걸러줬다.
...
- **Q. 동시성 버그는 어떻게 찾고 검증하셨나요?**
  → "재검증하려고 넣은 진단 코드 자체가 깨져서 나온 게 결정적 단서였습니다. 여러 스레드가 같은 `std::cout`에 동시에 쓰면서 문자 단위로 섞이는 걸 직접 눈으로 봤고, 그 시점에 `sync_with_stdio(false)`가 스레드 안전장치를 없앴다는 걸 되짚어 원인을 확정했습니다. 고친 뒤엔 '레이스만 없앤 건지, 성능까지 바뀐 건지'를 구분하려고 수정 전/후 성능 지표를 다시 실측해서 비교했습니다 — 지표가 그대로라는 걸 확인하고서야 순수 정확성 버그였다고 결론 내릴 수 있었습니다."
- **Q. "실행 검증은 선택 사항"이라고 미뤄뒀던 부분에서 버그가 나온 건 어떻게 보시나요?**
  → "정확히 제가 우려했어야 할 지점이었다고 생각합니다. 2순위·3순위·5순위 작업 때 '빌드/단위테스트는 통과했으니 코드 관점에선 완료, 실제 브라우저로 열어보는 건 선택 사항'이라고 판단했는데, 실제로 열어보니 시작 백그라운드 서비스 하나가 앱 전체를 크래시시키는 버그가 있었습니다. 단위테스트는 EF Core의 LINQ-투-SQL 번역 실패까지는 안 잡아준다는 게 이번에 드러난 맹점이었고, 앞으로는 '단위테스트 통과'와 '실제로 안 죽고 뜬다'를 분리해서 확인하는 습관을 들이게 됐습니다."
- **Q. 같은 버그가 다른 곳에도 있는지는 어떻게 확인하셨나요?**
  → "`Traces` 컨트롤러에서 같은 원인의 500 에러를 발견한 직후, '이 프로젝트에 DateTimeOffset 부등호 비교를 쓰는 다른 쿼리가 더 있는가'를 코드 검색으로 확인했습니다. `AlertsController`/`AlertEvaluator`도 시간 관련 조건을 쓰긴 하지만 전부 `== null`/`!= null` 같은 동등 비교라 이 버그의 영향을 안 받는다는 걸 확인하고서야 안심할 수 있었습니다 — 버그 하나를 고치고 끝내는 게 아니라 '같은 패턴이 또 있는가'를 능동적으로 재검색하는 게 중요하다고 생각합니다."
