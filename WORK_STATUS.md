...
8순위(신규 트랙) : Qt/MFC 포트폴리오 확장               🟡 진행 중(2026-09-17) — `APM_Agent` Phase A 완료, Qt 2′~7′ 전부 적용+실측 검증 완료(6′는 커밋 `866cd93` 완료, 7′는 적용+검증 완료 커밋 대기). 8단계(배포판 구성)는 사용자 판단으로 스킵, 대신 `portfolio_apm_light.html`에 Qt 트랙 신규 섹션 반영 완료(커밋 대기)
```
...
**다음 할 일**: 커밋 여부는 사용자 요청 시. 이후 6′(알림+연결상태)/7′(Agent 제어 UI) 진행. **(2026-09-17 갱신: 커밋/push는 사용자가 완료함, 아래로 계속됨)**

**6′ 설계 재검토 완료(2026-09-17, 같은 세션 이어서)** — 2026-09-14에 작성해둔 6′ 원안은 그 시점의 폴링 기반 `MainWindow` 위에서 짜여 있었는데, 그 사이 발행-구독 재설계(위 항목)가 먼저 적용되며 기준 자체가 낡음 → 사용자 "6 설계 재검토 진행하자" 요청으로 현재 실제 코드(`4f93d3b`) 기준으로 다시 맞춤.

**이번에 새로 확정한 것 — 알림/상태 갱신 트리거**: Agent가 Collector에 보내기 전 이미 로컬로 알림을 판단해두므로, Collector 푸시 시점엔 대부분 최신 상태 → **알림/상태 전용 푸시 채널을 새로 만들지 않고, 기존 지표 갱신 트리거(Collector 푸시 우선 + 30초 안전망)에 그대로 얹기로 확정**(사용자 "추천대로 진행하자"). 감수하는 트레이드오프: Agent↔Collector 연결이 끊긴 동안은 알림/상태 UI도 30초 안전망 주기로만 갱신됨(소켓을 하나 더 늘리는 비용이 더 크다고 판단).

핵심 설계(스키마/`AgentAlertRepository`/상태 판정 기준: `agent_status.updated_at`이 15초 넘게 안 갱신되면 "응답 없음")는 2026-09-14 원안과 동일. `Docs/SESSION_LOG.md` 2026-09-17 "6′ 설계 재검토(발행-구독 반영 후) — 확정 및 전체 코드 제안" 항목에 현재 코드(`MetricsWorker`/`MainWindow`/`CMakeLists.txt`/`main.cpp`) 기준 수정 전/후 전체 diff 작성(제안만, 소스 미작성 — 원칙 2).

**6′ 직접 적용 완료(2026-09-17, 같은 세션 이어서)** — 사용자 "적용하고 다음으로 넘어가자" 요청(원칙 2 예외). SESSION_LOG 제안 그대로 `AgentAlertRepository.h/.cpp`(신규) + Agent 쪽(`AgentAlertStore`/`ResilientSender::IsConnected()`/`Agent/main.cpp`) + Qt 쪽(`MetricsWorker`/`MainWindow`/`CMakeLists.txt`/`main.cpp`) 전부 직접 작성. 적용 중 내 실수로 `Agent/main.cpp` 스케줄러 콜백 람다에 `&alertStore` 캡처를 빠뜨려 컴파일 에러가 났고, 1줄 수정으로 해결(사용자 작성분 문제 아님).

**검증(직접, 실제 Collector+Agent+Qt 3개 프로세스 실행)**: `APM_Agent`/`APM_QtDashboard` 둘 다 클린 빌드 성공. 6초 실행 관찰 중 `agent_alerts.db`의 `agent_status` 테이블이 `connected=1`로 매 수집 주기마다 정상 갱신되는 것을 Python sqlite3로 직접 확인, Collector 로그에 구독자 연결 확인, Qt 헤드리스 로그에 경고/에러 없음. **단, Collector/Agent 다운 시나리오(안전망 30초 갱신, 15초 응답없음 전환, DB 자체가 없는 경우)는 시간 제약으로 이번엔 생략** — 다음 세션에서 확인 필요. 상세: `Docs/SESSION_LOG.md` 2026-09-17 "6′(알림+연결상태) 직접 적용 완료" 항목.

**남은 검증 3건 대행 완료(2026-09-17, 같은 세션 이어서)** — 사용자 "검증 대행 가능한가?" 질문에 직접 실행해서 확인. 라벨 텍스트 확인을 위해 `QT_QPA_PLATFORM=offscreen` + `QWidget::grab()` 임시 스크린샷 코드를 `main.cpp`에 잠깐 넣었다가 검증 후 완전히 되돌림(클린 빌드로 재확인).

- **버그 발견+수정**: `MetricsWorker::Refresh()`가 `_alertRepository.IsOpen()`일 때만 알림/상태를 조회하도록 가드돼 있었는데, Agent가 한 번도 안 떠서 `agent_alerts.db` 자체가 없으면 `Open()`이 영원히 실패해 이 가드가 항상 막혀버려 라벨이 "알 수 없음"이 아니라 초기 문구 "확인 중..."에 멈춰있는 버그였음. 가드를 제거해 항상 조회를 시도하도록 수정(내부에서 실패를 이미 안전하게 처리하므로 문제 없음).
- **① Agent 미기동**: 화면에 "Agent 상태: 알 수 없음(agent_alerts.db에 상태 없음)" 정상 표시(버그 수정 후).
- **② Agent 다운(Collector는 살아있음)**: 22초 후 "Agent 상태: 응답 없음 (마지막 갱신 22초 전)" 정상 표시, `agent_status.updated_at` 정지 확인.
- **③ Collector 다운(Agent는 살아있음)**: 38초 후 "Agent 상태: Collector 연결 끊김(재시도 중)"로 정상 전환 — **30초 안전망 타이머가 푸시 없이도 실제로 갱신을 계속 트리거함을 직접 증명**. Agent 로그에서 재연결 시도/큐잉도 계속 정상 동작 확인.

전부 실제 프로세스 3개 + 화면 캡처 + DB 직접 조회로 확인(추측 아님). 상세: `Docs/SESSION_LOG.md` 2026-09-17 "6′ 남은 검증 3건 대행 완료(다운 시나리오)" 항목.

**다음 할 일**: 커밋/push는 사용자 요청 시. 이제 §7(Agent 제어 UI, `QLocalSocket`) 착수 가능.

**커밋+push 완료(2026-09-17, 같은 세션 이어서)** — 사용자 요청으로 6′ 관련 변경 전부(`866cd93` "Qt track: complete stage 6' (Agent alerts + connection-status display)") 커밋 후 `origin/master`에 push 완료.

**§7(Agent 제어 UI) 설계·코드 제안 완료(2026-09-17, 같은 세션 이어서)** — 사용자가 "지금 §7 설계 제안 준비"를 선택. Agent 쪽 `AgentControlServer`(Phase A에서 이미 완성, 변경 없음)의 핵심 제약 — **연결 하나당 명령 하나만 처리하고 끝남**(루프 없음) — 을 재확인하고, Qt 쪽 신규 `AgentControlClient`(매 명령마다 새 `QLocalSocket` 연결)를 설계. `MainWindow`에 버튼 3개(시작/중지/강제 재연결)+로그레벨 콤보+적용 버튼+결과 라벨을 추가하는 전체 코드를 `Docs/SESSION_LOG.md` 2026-09-17 "§7(Agent 제어 UI) 설계·코드 제안" 항목에 작성(제안만, 소스 미작성 — 원칙 2). `QJsonDocument`/`QJsonObject`는 `Qt6::Core`에 이미 포함돼 있어 CMake 모듈 추가 불필요.

**§7 직접 적용 완료(2026-09-17, 같은 세션 이어서)** — 사용자 "이것도 그냥 바로 적용해주고 문서화에 신경써줘" 요청(원칙 2 예외). SESSION_LOG 제안 그대로 `AgentControlClient.h/.cpp`(신규) + `MainWindow`/`CMakeLists.txt` 전부 직접 작성 — 편차 없이 제안 그대로 적용됨(6′ 때와 달리 사용자가 미리 손댄 부분이 없었음).

**검증(직접, 실제 Collector+Agent+Qt 3개 프로세스 실행 + 헤드리스 UI 자동 조작)**: 클린 빌드 2회 성공(증분 1회 + `rm -rf build` 전체 재구성 1회). `QMetaObject::invokeMethod`로 `MainWindow`의 버튼 슬롯을 헤드리스로 직접 호출해 4개 명령(시작/중지/강제 재연결/로그레벨) 전부 실행 — Agent 로그에서 `paused`/`resumed`/`force reconnect` 실제 반영 확인, Qt 화면엔 4개 전부 "명령 성공"으로 표시. **Agent가 안 떠 있는 상태(크래시 재현)에서도 무응답으로 안 멈추고 "명령 실패: Connection refused"로 즉시 표시**되는 실패 경로까지 확인. 상세: `Docs/SESSION_LOG.md` 2026-09-17 "§7(Agent 제어 UI) 직접 적용 완료" 항목.

**문서 갱신**: `Docs/QT_MFC_PORTFOLIO_PLAN.md` §6(공수 견적 새 표)에 2′~7′ 전 단계 완료 상태(✅)와 근거 커밋/검증 요약 반영, §9(진행 순서)도 1~3번 완료 표시로 갱신.

**다음 할 일**: §7 커밋/push는 사용자 요청 시(이번 요청엔 "적용"만 있었고 커밋 지시는 없었음). 완료되면 Qt/MFC 포트폴리오 트랙 8순위 남은 건 8단계(배포판 구성)뿐.

**8단계(배포판 구성) 스킵 확정 + 포트폴리오 갱신으로 전환(2026-09-17, 같은 세션 이어서)** — 사용자 "배포판 구성은 필요없다 이제 기존의 포트폴리오를 이에 맞게 수정하는 것이 더 중요하다". 범위 확인(AskUserQuestion) 결과: 이번엔 `Docs/portfolio_apm_light.html`만 반영(dark 버전/두 PPTX는 다음 별도 요청), 반영 방식은 기존 섹션 재작성이 아니라 **신규 섹션 하나 추가**.
...
**`김서현_APM_포트폴리오_v1.pptx`(구버전)도 반영 완료(2026-09-17, 같은 세션 이어서)** — 사용자 "PPTX 갱신해라". 이 덱은 `portfolio_apm_light.pptx`보다 훨씬 이른 시점(2026-07-26)에 멈춘 별도 디자인(4:3, 카드별 색상 다름, 스케일 테스트/실제 스크린샷 등 다수 누락)이라 Qt 트랙 전체 최신화는 범위 밖으로 판단, **"설계 의사결정 01–03/04–06" 패턴을 그대로 이어 "07–09" 슬라이드 1장만 추가**(아키텍처 대전환/알림 판단 위치/발행-구독 재설계). `validate.py` 통과+렌더링 확인 완료. 상세: `Docs/SESSION_LOG.md` 2026-09-17 "포트폴리오(구버전 v1 pptx) 갱신" 항목.

**다음 할 일**: 이 v1 덱의 나머지 격차(스케일 테스트/버그#3·4/실제 스크린샷 등)는 여전히 범위 밖 — 필요 시 별도 요청. 커밋/push는 사용자 요청 시. 이로써 Qt/MFC 포트폴리오 트랙(8순위) 및 포트폴리오 산출물 4종(html×2, pptx×2) 전부에 Qt 트랙 반영 완료.

### 포트폴리오 작업 기준 확정 — `portfolio_apm_light`(html/pptx)로 전환 (2026-09-18)

**배경**: 사용자가 "현재 ppt는 김서현_APM_포트폴리오_v1.pptx를 기준으로 작업하겠다"고 제안 → 격차 분석 결과를 보고한 뒤(아래), "portfolio_apm_light가 더 최신화된 내용이란 것인가?"로 재확인 → **작업 기준을 `portfolio_apm_light.html`/`.pptx`로 전환하기로 확정**. `김서현_APM_포트폴리오_v1.pptx`는 2026-07-26 초기 스냅샷 이후 방치된 구버전(2026-07-29 기록에도 "그 이후 작업 전부 반영 안 된 구버전이라 참고 안 함" 명시)이었던 게 재확인됨 — 이후 포트폴리오 관련 작업은 **`portfolio_apm_light.html`(원본, 계속 갱신되는 살아있는 문서) 기준으로 진행**하고, `portfolio_apm_light.pptx`는 그 html을 따라가는 발표용 산출물로 취급한다. `김서현_APM_포트폴리오_v1.pptx`/`portfolio_apm.html`(dark)은 이제부터 보조 산출물(필요 시 별도 요청으로 동기화)로 격하.

**v1.pptx 격차 분석 결과(2026-09-18, 기준 전환 전 수행)** — v1은 `portfolio_apm_light` 대비 다음이 통째로 빠져 있었음: 스케일 테스트 5라운드(72→300/300, WAL 트레이드오프, futex 병목, Collector 스레드 배치), 버그 #3(EF Core+SQLite `DateTimeOffset` 크래시 — `RetentionService` 크래시 + `/apm/traces` 500 에러 두 곳), 버그 #4(콘솔 로그 동시쓰기 레이스), 2/5순위 실행 검증 결과(알림/트레이스 실제 브라우저·curl 검증), 실제 스크린샷(플레이스홀더만 있었음). 수치 오류도 있었음(테스트 수 "17"→실제 27, 이건 v1엔 여전히 미수정 상태로 남음 — 범위 밖 확정 이후이므로).

**`portfolio_apm_light` 자체의 격차 분석(2026-09-18)** — 이번 기준 전환을 계기로 `portfolio_apm_light`(현재 기준)도 재점검, 아래 3건 발견:

1. **MFC 트랙이 포트폴리오에 전혀 없음** — 저장소에 `APM_Viewer/`(MFC 다이얼로그 기반 뷰어, `Docs/QT_MFC_PORTFOLIO_PLAN.md`가 "MFC Viewer" 참조 근거로 계속 인용)가 있고 트랙 이름 자체가 "Qt·**MFC**" 포트폴리오 확장인데도, `portfolio_apm_light.html`/`.pptx`엔 "MFC"라는 단어 자체가 한 번도 안 나옴. 단, `Docs/QT_MFC_PORTFOLIO_PLAN.md` §4 자체가 "MFC는 주력이 아니다, 별도 학습에 시간 안 쓴다 — '만들어봤다' 수준 포지셔닝 + 면접에서 판단만 설명" 방침을 이미 명시하고 있어(과거 다른 면접에서 "스크린샷 2장 + 구조 설명"으로 통했다는 기록도 있음), 이게 "빠뜨린 것"인지 "의도적으로 최소화한 것"인지는 판단이 필요함 — 최소한 한두 줄 언급/스크린샷 정도는 추가할 여지가 있음. **또한 `APM_Viewer/APM_Viewer/SqliteReader.cpp`/`ApmViewerDlg.cpp`가 실제로는 아직 TODO 스켈레톤 상태**(완성 예제는 `APM_Viewer/SESSION_LOG.md`에 참고용으로만 있음, 프로젝트 파일 자체엔 미적용)라 — 지금 포트폴리오에 넣을 만큼 완성된 상태가 아님(코드 완성이 선행돼야 함).
2. **버그 넘버링 충돌** — `portfolio_apm_light.html`의 "버그 발견" 섹션에 `// Bug #3`가 **두 번** 나옴(하나는 "Razor HTML 인코딩", 하나는 "EF Core+SQLite DateTimeOffset") — 번호가 겹쳐 있음. 실제로는 Lock.cpp(#1)/콘솔레이스(#2)/Razor(#3)/TLS(#4)/EF Core(#3 중복, 실제론 #5여야 함) 순서로 보임 — 경미하지만 번호를 다시 매겨야 함.
3. **Qt 트랙 자체에서 발견한 버그들이 "버그 발견" 섹션에 아직 안 들어감** — 이번 Qt 트랙 작업 중 실제로 발견·수정한 버그가 최소 3건 있음: (a) `MetricsBroadcastServer.h/.cpp` 오타 5건(사용자 작성분, 발행-구독 재설계 적용 중 발견), (b) `Agent/main.cpp` 람다 `&alertStore` 캡처 누락(§7 적용 중 내가 만든 실수), (c) "Agent 미기동" 상태에서 라벨이 "확인 중..."에 멈추던 버그(`MetricsWorker::Refresh()`의 `IsOpen()` 가드 문제, 6′ 검증 중 발견). 지금 새로 만든 `#qt-track` 섹션엔 (c)만 "판단"란에 짧게 언급돼 있고, 별도의 "버그 스포트라이트" 카드로는 안 들어가 있음 — 기존 버그 섹션과 같은 형식으로 추가할지는 판단 필요(다른 버그들에 비해 상대적으로 작은 버그라 스포트라이트급인지는 애매함).

**다음 할 일**: 위 3건 중 어느 것을 반영할지(전부/일부/보류) 지시 대기. 숫자 오류(버그 #3 중복)는 사실 관계상 명백한 수정 대상이라 우선순위가 높다고 판단됨 — 나머지 2건은 판단 필요.
...
**"GitHub 원격 브랜치 불일치" 해소 + push 완료(2026-09-07, 이어지는 세션)** — 사용자 요청으로 push 진행 중, `git fetch`에서 `origin/main`이 그 사이 한 번 더 앞으로 나가 있고(`08d5875` "Update : MFC Collector UI" — 다른 PC에서 만든 신규 MFC Viewer 프로젝트, 로컬엔 전혀 없던 내용) 로컬 `master`와는 여전히 공통 조상이 없는 별개 히스토리임을 재확인. 사용자에게 처리 방식을 확인(AskUserQuestion) → "그 커밋 보존하며 병합" 선택:
- `git cherry-pick 08d5875` — `APM_Viewer/` 전체가 로컬에 없던 새 디렉터리라 충돌 없이 깔끔하게 적용됨(`6c489a0`). 삭제된 `apm_metrics.db-journal`도 로컬에선 이미 2026-07-28 WAL 커밋(`2d44c72`)에서 추적 해제된 상태라 실질적 충돌 없음.
- push 방식도 재확인(AskUserQuestion) → "force-push 진행" 선택. `git push origin master:main --force-with-lease=main:08d5875...`로 `origin/main`을 로컬 `master`(MFC Viewer 커밋 포함)로 갱신 — 내용 손실 없음, 원격 `main`의 커밋 해시만 재작성됨. 이어서 `git push origin master`로 `origin/master`도 동일 지점까지 fast-forward.
- 결과: `origin/main` == `origin/master` == 로컬 `master` == `90d5223` "Note the previous commit in WORK_STATUS.md"로 전부 일치. 브랜치 불일치 이슈 해소.

**남은 것**: Windows PC 등 다른 체크아웃은 다음에 이 저장소를 받을 때 `origin/main`의 히스토리가 재작성됐으므로 기존 로컬 브랜치를 그대로 `pull`하면 충돌 — `git fetch` 후 `git reset --hard origin/main`(또는 새로 clone) 필요. 사용자에게 이 점 안내 필요.
